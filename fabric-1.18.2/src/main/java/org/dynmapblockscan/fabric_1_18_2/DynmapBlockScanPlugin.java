...
import org.dynmapblockscan.fabric_1_18_2.statehandlers.FabricStateContainer;
...
public class DynmapBlockScanPlugin extends AbstractBlockScanBase
{
    ...
    public void buildAssetMap() {
        ...
        List<ModContainer> mcl = flInst.getAllMods().stream().toList();
    ...
    public void serverStarting() {
    	...
    	DefaultedRegistry<Block> bsids = Registry.BLOCK;
        Block baseb = null;
        ...
        while (iter.hasNext()) {
            ...
            if (b == baseb) { continue; }
            baseb = b;
            RegistryKey<Block> rl = Registry.BLOCK.getKey(b).orElseThrow();
...
//            			break;
            ...
            Material mat = b.getDefaultState().getMaterial();
        	...
        	if (blockstate != null) {
                ...
                try {	// Workaround for mods with broken block state logic...
                	br.lightAttenuation = b.getDefaultState().isSolidBlock(EmptyBlockView.INSTANCE, BlockPos.ORIGIN) ? 15 : (b.getDefaultState().isTranslucent(EmptyBlockView.INSTANCE, BlockPos.ORIGIN) ? 0 : 1);
                } catch (Exception x) {
